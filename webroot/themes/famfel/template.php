 ...
 *  the prefix from theme_ to famfel_.  For example:
 *
 *   original:  theme_breadcrumb() 
 *   theme override:   famfel_breadcrumb()
 *
 ...
 **/
 function famfel_breadcrumb($breadcrumb) {
   if (!empty($breadcrumb)) {
...
function _phptemplate_variables($hook, $vars = array()) {  
  ...
  switch ($hook) {
    ...
    case 'page':
	  	...
	  	else
		{
		      ...
		      if ($theme == $theme_key) {
		        ...
		        //drupal_add_css($vars['directory'] .'/icons.css', 'theme', 'all');
		        //drupal_add_css($vars['directory'] .'/zen.css', 'theme', 'all');
				drupal_add_css($vars['directory'] .'/style/override.css', 'theme', 'all');
				drupal_add_css($vars['directory'] .'/style/core.css', 'theme', 'all');
				drupal_add_css($vars['directory'] .'/style/sIFR-screen.css', 'theme', 'all');
				drupal_add_js($vars['directory'] .'/scripts/sifr.js', 'theme');
				drupal_add_js($vars['directory'] .'/scripts/sifr-addons.js', 'theme');
				drupal_add_js($vars['directory'] .'/scripts/swfobject.js');
		        $vars['css'] = drupal_add_css($vars['directory'] .'/print.css', 'theme', 'print');
		        $vars['styles'] = drupal_get_css();
				$vars['scripts'] = drupal_get_js();
		      }
      ...
      if ($vars['node']->type) {
        ...
        $body_classes[] = 'ntype-'. famfel_id_safe($vars['node']->type);
      }
      switch (TRUE) {
    ...
    case 'node':
      ...
      $node_classes[] = 'ntype-'. famfel_id_safe($vars['node']->type);
      // implode with spaces
  ...
  if (function_exists('famfel_variables')) {
    $vars = famfel_variables($hook, $vars);
  }
...
*/
function famfel_id_safe($string) {
 ...
 */ 
function _famfel_default($hook, $variables, $suggestions = array(), $extension = '.tpl.php') {
  global $theme_engine;
  global $theme;
  global $theme_key;
